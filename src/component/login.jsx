
import { Stack,Input,Box, Button ,Image} from "@chakra-ui/react"
import axios from "axios"
import { useState } from "react"
import { useNavigate } from "react-router-dom"
const initial={
    email:"",
    password :""
}
export default  function Login(){
const [formdata,setFormdata] = useState(initial);
const [token,setToken] = useState("");
const navigate = useNavigate()
const handleChange=(e)=>{
const {value,name} = e.target;

setFormdata({...formdata,[name]:value})

}
const handleSubmit=(e)=>{
e.preventDefault()
console.log(formdata)
axios.post(`https://reqres.in/api/login`,{
    username: formdata.email,
    password :formdata.password
})
.then((res)=>
setToken(res.data.token)
)

}
if(token){
    alert("Success")
    return navigate("/")
}
const {email,password} = formdata

    return(
        <div style={{backgroundColor:"#fdefea " , height:"800px"}}>

       
        <div style={{width:"30%",height:"500px",border: "1px solid transparent",margin:"auto",
        backgroundColor:"#fdefea "}}>
          <Stack  >
        
       <Box>
        <h2>Enter Account Password</h2>
       </Box>
<Box>
    <h4>For Additional Securuties , Enter your <br/> Account Details</h4>
</Box>


<Box>
    <form onSubmit={handleSubmit}>
        <Box>
     <Input   placeholder="Email"   name="email" type="email" value={email}
     onChange={handleChange} style ={{height:"30px", width:"80%" , marginTop:"15px"}}   />
        </Box>

        <Box>
     <Input   placeholder="Password"   name="password" type="password" value={password}
     onChange={handleChange}  style ={{height:"40px", width:"80%" , marginTop:"15px"}}  />
        </Box>

        <Box>
     <Input type="submit" style ={{height:"40px" ,width:"80%"
     
     , marginTop:"15px"  , fontSize:"800" ,backgroundColor:"#ff3f6c" , borderRadius:"6px",
     
         borderColor:"transparent" , color :"white"                                          }} />
        </Box>

    </form>


    <Box>
        <h5>Have a trouble in Login ? Get help<br/> or continue with </h5>
    </Box>

<Box>
    <Image 
    src ="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSNWNBSKHopKXxA6Jm0WBZWq6tvVzJ6P4jBCQ&usqp=CAU" 
    
      style={{height:"55px" , borderRadius:"50%" }}          />
 <Image 
    src ="https://upload.wikimedia.org/wikipedia/commons/thumb/1/1b/Facebook_icon.svg/2048px-Facebook_icon.svg.png" 
    
      style={{height:"55px" , borderRadius:"50%" }}          />
<Image 
    src ="data:image/png;base64," 
    
      style={{height:"55px" , borderRadius:"50%" }}          />

</Box>


<Box>
   <Button  className="btn"   style ={{height:"40px", width:"80%" 
   , marginTop:"15px" , borderRadius:"6px"}}
   >CREATE NEW ACCOUNT</Button>
</Box>
</Box>


      </Stack> 
        </div>
        </div>
    )
}